...
def validate_tariff_code_columns(df):
    ...
    # POD values where tariff code must be BLANK (your new rule)
    ...
    # Treat pure whitespace as blank (you already effectively do this via strip)
    has_tariff = tariff_raw.notna() & (tariff != "")

    # Optional hygiene: remove BOM and NBSP before validation # might've remove this in later version
    # (BOM can stop a valid prefix matching; NBSP sometimes survives copy/paste from Excel)
    tariff_clean = (
        tariff
    ...
    # Case A (your required behaviour):
    # POD is in exclude list => tariff MUST be blank
    excluded = pod_known & pod.isin(exclude_pods)
    invalid_present_when_excluded = excluded & has_tariff

    # Case B:
    # POD is NOT in exclude list => tariff must be populated and start with HRG
    ...
    invalid_mask = (
        invalid_too_long
        | invalid_present_when_excluded
        | invalid_missing_when_required
        | invalid_bad_prefix_when_required)
