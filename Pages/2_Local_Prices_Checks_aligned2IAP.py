...
if st.session_state.upload_success:
    st.success("Local Prices CSV file uploaded successfully!")
...
def validate_activity_TFC_columns(df):
    tfc_col = 'ACTIVITY TREATMENT FUNCTION CODE'
    pod_col = 'POINT OF DELIVERY CODE'

    for c in (tfc_col, pod_col):
    ...
    valid_codes = set(tfc.iloc[:, 0].dropna().astype(str).str.strip())

    tfc_raw = df[tfc_col].astype("string")
    tfc = tfc_raw.str.strip()
    ...
    has_tfc = tfc_raw.notna() & (tfc != "")
    is_non_activity = pod.isin(NON_ACTIVITY_PODS)
    is_activity = ~is_non_activity

    # Rule 1: must be blank for non‑activity PODs
    invalid_when_non_activity = is_non_activity & has_tfc

    # Rule 2: mandatory + must be valid for activity PODs
    invalid_missing_when_required = is_activity & (~has_tfc)
    invalid_not_in_ref = is_activity & has_tfc & (~tfc.isin(valid_codes))

    invalid_rows = df[
        invalid_when_non_activity |
            ...
            with st.spinner("Running calculations..."):
                ...
                notes = columns.map(
                    lambda c:
                        BLANK_RULE_NOTE
                        if (
                            c in BLANK_WHEN_NON_ACTIVITY_POD_FIELDS
